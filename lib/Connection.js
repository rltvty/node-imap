var tls = require('tls'),
    ...
    utf7 = require('emailjs-utf7').imap;
...
Connection.prototype.connect = function() {
  ...
  this._onClose = function(had_err) {
    clearTimeout(self._tmrConn);
    clearTimeout(self._tmrAuth);
    clearTimeout(self._tmrKeepalive);
    self.state = 'disconnected';
  ...
  this._sock.once('close', this._onClose);
  if (config.tls) {
    socket.once('close', this._onClose);
  }

  this._onEnd = function(err) {
    clearTimeout(self._tmrConn);
    clearTimeout(self._tmrAuth);
    clearTimeout(self._tmrKeepalive);
    self.state = 'disconnected';
    self.debug && self.debug('[connection] Ended: ' + err);
    if (err) {
      err.source = 'socket';
      self.emit('error', err);
    } else {
  ...
  this._sock.once('end', this._onEnd);
  if (config.tls) {
  ...
  parser.on('error', function(error) {
    self._curReq.bodyEmitter.emit('error', error);
  });
