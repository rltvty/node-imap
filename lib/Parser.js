...
function parseExpr(o, literals, result, start, useBrackets, isEnvelope) {
  ...
  for (var i = start, len = o.str.length; i < len; ++i) {
    if (isEnvelope && indexToDisableIsEnvelope == result.length) {
      isEnvelope = false;
      indexToDisableIsEnvelope = null;
    }
    ...
    if (!inQuote) {
      if (isBody) {
        ...
        if (i - (lastPos + 1) > 0) {
          ...
          if (val === 'ENVELOPE' && !isEnvelope) {
            isEnvelope = true;
            indexToDisableIsEnvelope = result.length + 2
          }
          result.push(val);
        }
...
function parseEnvelope(envelope) {
  ...
  for (var i = 0, len = envelope.length; i < len; ++i) {
    if (result.length == 9) { //now working on messageId
      val = envelope.substr(lastPos + 2, len - lastPos - 3);
      console.log('pushing remainder: ' + val);
      result.push(val);
      break; //out of for loop
    }
    console.log('Now processing char: ' + envelope[i]);
    if (escaping) {
      escaping = false;
      console.log('escaping is now: ' + escaping);
    ...
    switch (envelope[i]) {
      ...
      case '"':
        inQuote = !inQuote;
        console.log('inQuote is now: ' + inQuote);
        break;
      case "(":
        parenthesesCount += 1;
        console.log('parenthesesCount is now: ' + parenthesesCount);
        break;
      case ")":
        ...
        break;
      case ' ':
        if (!inQuote && parenthesesCount == 0) {
          val = envelope.substr(lastPos, i - lastPos);
          var innerResult = parseExpr(val);
          console.log('innerResult' + inspect(innerResult));
          console.log('pushing: ' + val);
          result.push(innerResult[0]);
          lastPos = i;
        }
        break;
      default:
